...
def client_with(handler, **kwargs):
    return IntervalsClient("i12345", FAKE_KEY, transport=httpx.MockTransport(handler), **kwargs)
...
def test_client_only_ever_sends_get():
    ...
    def handler(request):
        ...
        if request.url.path.endswith("/streams.json"):
            return httpx.Response(200, json=latlng_stream([33.4], [-84.6]))
        return httpx.Response(200, json=[])
...
def test_latlng_request_and_parse():
    def handler(request):
        ...
        return httpx.Response(200, json=latlng_stream([33.39, None, 33.40], [-84.57, None, -84.58]))
...
def test_list_must_be_a_list():
    c = client_with(lambda r: httpx.Response(200, json={"error": "nope"}))
...
def test_persistent_errors_raise_after_retries():
    slept = []
    c = client_with(lambda r: httpx.Response(503), sleep=slept.append, tries=3)
